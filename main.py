...
def apriori_join_prune(transactions, min_support_count):
    frequent_itemsets = []
    k = 1
    # Get unique items in the transactions
    ...
    while True:
        ...
        # Calculate support counts for candidate itemsets
        support_counts = calculate_support_count(candidate_itemsets, transactions)

        # Prune step: Select candidate itemsets with support count >= min_support_count
...
if __name__ == "__main__":
    # Define the transactional database
    transactions = [
        ["A", "B", "E"],
        ["B", "D"],
        ["B", "C"],
